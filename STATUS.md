...
- Issue 0019 (AI eval dataset) is **Done**: `feat/ai-eval-dataset` merged into `main` as
  ...
  cases (20 crop, 6 variety) and `evals/README.md` states the global invariants. There
  is **no runner** yet: nothing executes the cases against the model; that is the next
  eval issue. The dataset predates the 0020 prompt change (excluded list is now
  server-filled), so case expectations about `excluded` reasons should be re-read
  before a runner is written.
