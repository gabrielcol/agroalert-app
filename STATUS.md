...
- Issue 0021 (AI eval test suite + opt-in live runner) is **Done**: `feat/ai-eval-runner`
  merged into `main` on 2026-09-13 (locally, `gh` unauthenticated). `evals/dataset.test.ts`
  runs in `bun run test` and applies every README "Validating the dataset" check to the 26
  cases and 9 briefs; `evals/lib/` holds the case schema, loaders, static checks, Romanian
  heuristic and the invariant / `expect` assertions (95 tests). `bun run eval` (`evals/run.ts`,
  flags `--filter --kind --model --concurrency`) calls `recommendCrops` / `rankVarieties`
  directly on the frozen briefs and writes a JSON report to the gitignored `evals/results/`;
  exit 2 without `ANTHROPIC_API_KEY`, 1 on any failure. **First Haiku 4.5 baseline: 12/26
  passed** (8/20 crop, 4/6 variety), all failures model behaviour: soil-excluded crops
  (wheat / barley / rapeseed on argilos / nisipos) in the top (12 hits), `high` confidence
  on low-data cases (4), duplicate variety names (3), `mustInclude` crop missing (4), one
  merged sowing window, two fit orderings. Gate green; e2e exempt (dev tooling). Numbering:
  filed as 0021; the parallel session moved its two issues to 0022 / 0023.
- Issue 0022 (loading state for the varieties call: spinner + localized "loading
  varieties" status line above the skeleton while `recommendation.varieties` is pending)
  is **Done**: `fix/variety-loading-state` (from another session) merged into `main` as
  926aa72 on 2026-09-13, resolved against 0023 (CHANGELOG and `soi-screen.test.tsx` were
  combined; the not-ready Alert branch sits before the pending branch). Gate green on
  the merge (52 files / 346 tests); `e2e/variety-loading.spec.ts` written, **not run**.
- Issue 0023 (variety screen: dedupe the ranking, never render blank) is **Done**: merged
  into `main` as 414d54d on 2026-09-13 (locally, `gh` unauthenticated), from
  `fix/variety-screen-dedupe-blank-state` (renumbered from 0021, taken by the eval runner). The reported "empty variety screen" was **not**
  ...
  **has now run for real locally**: `ANTHROPIC_API_KEY` is set, `AI_MODEL=claude-sonnet-5`,
  ...
  ran are stale). The eval runner that was in flight alongside is issue 0021, now merged (see above).
...
- Issue 0019 (AI eval dataset) is **Done**: `feat/ai-eval-dataset` merged into `main` as
  fd42cc3 on 2026-09-13 (locally, `gh` unauthenticated, no PR; rule 3 exemption: no
  runtime surface). `evals/briefs/` holds nine frozen Weather Briefs plus `SUMMARY.md`
  (regenerated by `bun run capture:eval-briefs`), `evals/cases/` holds 26 hand-authored
  cases (20 crop, 6 variety) and `evals/README.md` states the global invariants. The
  runner and static suite landed in issue 0021. The dataset predates the 0020 prompt
  change (excluded list is now server-filled); invariant 1 is asserted on the returned
  result, which `completeExcluded` keeps complete.
...
0. Act on the eval baseline (issue 0021, 12/26 on Haiku): the prompt lets the model rank
   wheat / barley / rapeseed on `argilos` / `nisipos` soils and answer `high` confidence on
   unknown soil. Consider enforcing `unsuitable_classes` and the low-data confidence cap
   server-side (like `completeExcluded`), then re-run `bun run eval --model claude-haiku-4-5`.
